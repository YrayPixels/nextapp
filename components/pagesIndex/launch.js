...
function Programlaunching(props) {
    ...
    const fetchData = () => {
        const allFaculties = `${process.env.NEXT_PUBLIC_API_BASE}center/getAllFaculties`
        const allPrograms = `${process.env.NEXT_PUBLIC_API_BASE}admin/getAllProgrammes`
        const allSession = `${process.env.NEXT_PUBLIC_API_BASE}getAllSession/${details.id}`
    ...
    const programlaunch = async (e) => {
        ...
        function launchprog() {
            const programlaunch = async () => {
                const response = await fetch(`${process.env.NEXT_PUBLIC_API_BASE}center/LunchProgramme`, requestOptions)
                ...
                if (status == 200) {
                    ...
                    router.replace('/centers/launchcourse')
                } else {
