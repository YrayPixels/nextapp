...
function AllLaunchedProg(props) {
    ...
    const fetcher = async () => {
        const response = await fetch(`${process.env.NEXT_PUBLIC_API_BASE}center/GetAllLunchedProgrammeByCenterId/${details.id}`, requestOptions)
    ...
    function hideSession(param) {
        ...
        const fetcher = async () => {
            const response = await fetch(`${process.env.NEXT_PUBLIC_API_BASE}center/HideLunchedProgramme/${param}`, requestOptions)
            const data = await response.json()
            // setLoading(false)
                                ...
                                <tr className='align-items-center '>
                                    ...
                                    <td>
                                        <div className='btn-group'>
                                            ...
                                            <button onClick={() =>
                                                hideSession(`${programme.programme_lunched_id}`)} className='btn btn-sm btn-danger p-2'>
                                                Delete
                                            </button>
                                        </div>
